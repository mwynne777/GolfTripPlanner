...
router.get("/addCoordinatesToExistingCourse", async (req, res) => {
  console.log("Hit endpoint add coords to an existing course");
  const course = { id: 11263, townLat: 34.2676, townLong: -86.2089 };
...
router.get("/:id", async (req, res) => {
  ...
  const location = await fetch(
    `${geocodingUrl}${clubNameSerialized}}.json?access_token=${process.env.MAPBOX_ACCESS_TOKEN}`
  );
  if (!location.error) {
